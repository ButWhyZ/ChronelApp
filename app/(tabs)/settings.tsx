...
import { useAppTheme, ThemeMode } from "../../hooks/use-app-theme";
...
const STORAGE_KEYS = {
  ...
  // theme/accent handled by AppThemeProvider
};
...
export default function SettingsScreen() {
  ...
  const { colors, themeMode, accent, setThemeMode, setAccent } = useAppTheme();
  ...
  const confirmClearAll = () => {
    Alert.alert("Clear All Data", "This will remove all local data on this device.", [
  ...
  return (
    ...
    >
      ...
      <View style={[styles.card, { borderColor: colors.border, backgroundColor: colors.card }]} testID="settings_features_card_01">
        <ToggleRow
      ...
      <View style={styles.accentRow} testID="settings_accent_row_01">
        {ACCENT_OPTIONS.map((c, i) => (
          ...
          >
            {accent === c && (
              <Ionicons name="checkmark" size={22} color="#FFF" testID={`settings_accent_check_${i + 1}_01`} />
            )}
          </Pressable>
        ))}
      </View>
      ...
      <View style={[styles.privacyCard, { borderColor: colors.border, backgroundColor: colors.card }]} testID="settings_privacy_card_01">
        <Ionicons name="shield-checkmark-outline" size={22} color={colors.tint} testID="settings_privacy_icon_01" />
      ...
      <Pressable
        ...
        style={[styles.clearButton, { backgroundColor: "#C83737" }]}
...
function ThemeCard({
  ...
  return (
    <Pressable
      onPress={onPress}
      style={[styles.themeCard, { borderColor: selected ? colors.tint : colors.border, backgroundColor: colors.card }]}
      testID={`${id}_card_01`}
    >
...
const styles = StyleSheet.create({
  ...
  toggleRow: { flexDirection: "row", padding: 16, alignItems: "center" },
  toggleTitle: { fontSize: 18, fontWeight: "700" },
  toggleSubtitle: { fontSize: 14, marginTop: 4 },

  themeRow: { flexDirection: "row", gap: 12 },
  themeCard: { flex: 1, padding: 20, borderRadius: 18, borderWidth: 3, alignItems: "center" },
  themeLabel: { marginTop: 8, fontSize: 18, fontWeight: "700" },

  accentRow: { flexDirection: "row", gap: 16 },
  accentDot: { width: 56, height: 56, borderRadius: 999, alignItems: "center", justifyContent: "center" },

  privacyCard: { borderWidth: 1, borderRadius: 18, padding: 16, flexDirection: "row", gap: 12 },
  privacyText: { flex: 1, fontSize: 14, lineHeight: 20 },

  clearButton: { marginTop: 24, paddingVertical: 16, borderRadius: 18, alignItems: "center" },
